...
for (i in 1:length(samples)) {
  cnt <- read.table(paste0('fwd/', samples[i], '.fwd.cnt'))
...
#write.table(RPKM, 'table/virus_expression_RPKM_fwd.tsv', col.names = NA, sep = '\t', quote = F)

RPM <- round(RPM, 4)
colnames(RPM) <- samples
write.table(RPM, 'table/virus_expression_RPM_fwd.tsv', col.names = NA, sep = '\t', quote = F)
